SELECT
    ...
    ROUND(SUM(CASE WHEN activity_type = 'free_trial' THEN activity_duration ELSE 0 END) 
          / COUNT(DISTINCT CASE WHEN activity_type = 'free_trial' THEN activity_date END), 2) AS trial_avg_duration,
    ROUND(SUM(CASE WHEN activity_type = 'paid' THEN activity_duration ELSE 0 END) 
          / COUNT(DISTINCT CASE WHEN activity_type = 'paid' THEN activity_date END), 2) AS paid_avg_duration
FROM UserActivity
GROUP BY user_id
HAVING 
